...
class VideoDataset(Dataset):
    def __init__(
        ...
        output_video_column: str = "output_video",
        height: int = 480,
        width: int = 720,
        ...
        if self.input_num_instance_videos != len(self.instance_prompts):
            raise ValueError(
                f"Expected length of instance prompts and videos to be the same but found {len(self.instance_prompts)=} and {len(self.input_instance_video_paths)=}. Please ensure that the number of caption prompts and videos match in your dataset."
            )
        if self.input_num_instance_videos != self.output_num_instance_videos:
            raise ValueError(
                f"Expected length of input and output instance videos to be the same but found {self.input_num_instance_videos=} and {self.output_num_instance_videos=}. Please ensure that the number of input and output videos match in your dataset."
            )

        self.input_instance_videos, self.output_instance_videos = self._preprocess_data()

    def __len__(self):
        return self.input_num_instance_videos
    ...
    def _load_dataset_from_local_path(self):
        ...
        if not input_video_path.exists() or not input_video_path.is_file():
            raise ValueError(
                "Expected `--input_video_path` to be path to a file in `--instance_data_root` containing line-separated paths to video data in the same directory."
            )
        if not output_video_path.exists() or not output_video_path.is_file():
            raise ValueError(
                "Expected `--output_video_path` to be path to a file in `--instance_data_root` containing line-separated paths to video data in the same directory."
            )
        ...
        if any(not path.is_file() for path in input_instance_videos):
            raise ValueError(
                "Expected '--input_instance_videos' to be a path to a file in `--instance_data_root` containing line-separated paths to video data but found atleast one path that is not a valid file."
            )
        if any(not path.is_file() for path in output_instance_videos):
            raise ValueError(
                "Expected '--output_instance_videos' to be a path to a file in `--instance_data_root` containing line-separated paths to video data but found atleast one path that is not a valid file."
            )
    ...
    def _preprocess_data(self):
        ...
        progress_dataset_bar = tqdm(
            range(0, len(self.input_instance_video_paths)),
            desc="Loading progress resize and crop videos",
        )

        input_videos = []

        for filename in self.input_instance_video_paths:
            ...
            input_videos.append(frames.contiguous())  # [F, C, H, W]
            progress_dataset_bar.update(1)

        progress_dataset_bar.close()

        progress_dataset_bar = tqdm(
            range(0, len(self.output_instance_video_paths)),
            desc="Loading progress resize and crop videos",
        )

        output_videos = []

        for filename in self.output_instance_video_paths:
            video_reader = decord.VideoReader(uri=filename.as_posix())
            video_num_frames = len(video_reader)
            ...
            frames = self._resize_for_rectangle_crop(frames)
            output_videos.append(frames.contiguous())  # [F, C, H, W]
            progress_dataset_bar.update(1)

        progress_dataset_bar.close()


        return input_videos, output_videos
...
def main(args):
    ...
    progress_encode_bar = tqdm(
        range(0, len(train_dataset.input_instance_videos)),
        desc="Loading Encode Input videos",
    )
    train_dataset.input_instance_videos = [encode_video(video, progress_encode_bar) for video in train_dataset.input_instance_videos]
    progress_encode_bar.close()

    progress_encode_bar = tqdm(
        range(0, len(train_dataset.output_instance_videos)),
        desc="Loading Encode Output videos",
    )
    train_dataset.output_instance_videos = [encode_video(video, progress_encode_bar) for video in train_dataset.output_instance_videos]
    progress_encode_bar.close()
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            input_latent_dist = example["input_instance_video"]
            output_latent_dist = example["output_instance_video"]

            input_video_latents = input_latent_dist.sample() * vae.config.scaling_factor
            input_video_latents = input_video_latents.permute(0, 2, 1, 3, 4)

            output_video_latents = output_latent_dist.sample() * vae.config.scaling_factor
            output_video_latents = output_video_latents.permute(0, 2, 1, 3, 4)
            ...
            if random.random() < args.noised_image_dropout:
                input_video_latents = torch.zeros_like(input_video_latents)

            input_videos.append(input_video_latents)
            output_videos.append(output_video_latents)
        ...
        return {
            "videos": (output_videos, input_videos),
            "prompts": prompts,
        }
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                output_video_latents, input_video_latents = batch["videos"]
                prompt_embeds = batch["prompts"]

                input_video_latents = input_video_latents.to(dtype=weight_dtype)  # [B, F, C, H, W]
                output_video_latents = output_video_latents.to(dtype=weight_dtype)  # [B, F, C, H, W]

                batch_size, num_frames, num_channels, height, width = output_video_latents.shape

                # Sample a random timestep for each image
                timesteps = torch.randint(
                    0, scheduler.config.num_train_timesteps, (batch_size,), device=output_video_latents.device
                )
                ...
                noisy_video_latents = scheduler.add_noise(output_video_latents, noise, timesteps)
                noisy_model_input = torch.cat([noisy_video_latents, input_video_latents], dim=2)
